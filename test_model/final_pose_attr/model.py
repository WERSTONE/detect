...
class YOLOLikePoseAttrHead(nn.Module):
    ...
    def __init__(
        ...
        self.appearance_branches = nn.ModuleList(
            nn.Sequential(
                Conv(ch, attr_mid, 3),
                Conv(attr_mid, attr_mid, 3),
                nn.Conv2d(attr_mid, 2, 1),
            )
            for ch in self.channels
        )
    ...
    def _init_extra_bias(self):
        ...
        for branch in list(self.appearance_branches) + list(self.action_branches):
            nn.init.normal_(branch[-1].weight, 0.0, 0.01)
...
class DomainPoseAttrBiFPN(nn.Module):
    ...
    def compute_loss(self, images, gt_dict_list):
        ...
        det_idx = self._task_indices(gt_dict_list, "detect")
        pose_idx = self._task_indices(gt_dict_list, "pose")
        attr_idx = self._task_indices(gt_dict_list, "attr")

        if self.train_domain_det and det_idx:
            det_indices = torch.as_tensor(det_idx, device=device, dtype=torch.long)
            det_losses = self.domain_det_loss(
                self._slice_outs(outs["domain_det"], det_indices),
                self._filter_domain_gt([gt_dict_list[i] for i in det_idx], device),
            )
        else:
