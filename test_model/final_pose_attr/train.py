"""Trainer entry for the pose-anchor attribute redesign."""
...
from test_model.final_pose_attr.data import build_final_train_loader, build_final_val_loader
...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        default=str(PROJECT_ROOT / "test_model" / "final_pose_attr" / "yaml" / "final_pose_attr.yaml"),
    )
...
def main():
    ...
    base_save = Path(cfg["training"].get("save_dir", "checkpoints/final_pose_attr"))
    last_best = None
    ...
    if val_loader is not None and cfg.get("eval", {}).get("enabled", False):
        from test_model.final_pose_attr.eval import evaluate_checkpoint
