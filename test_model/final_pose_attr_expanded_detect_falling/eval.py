"""Pose-anchor attribute evaluation: domain mAP + attributes + COCO pose.

Runs the final ``DomainAttrBiFPN`` model on the val splits and reports:
  - domain detection mAP on Detect valid/
...
from test_model.final_pose_attr_expanded_detect_falling.data import (  # noqa: E402
    AttrDataset,
    DetectDataset,
    PoseDataset,
    _detect_source_configs,
...
from test_model.final_pose_attr_expanded_detect_falling.model import create_final_pose_attr_model  # noqa: E402
...
def build_model(cfg, weights, device):
    ...
    model = create_final_pose_attr_model(
...
def build_eval_loaders(cfg, split, batch, workers, input_size, max_samples=0):
    ...
    for key in det_keys:
        ...
        for source in _detect_source_configs(det_cfg):
            ...
            img_base, lbl_base = DETECT_DIR_MAP.get(key, ("valid", "valid"))
            img_dir = source.get(f"{key}_images", source.get("images", f"{img_base}/images"))
            lbl_dir = source.get(f"{key}_labels", source.get("labels", f"{lbl_base}/labels"))
            if (det_root / img_dir).exists() and (det_root / lbl_dir).exists():
                ...
                if key in {"train", "val"}:
                    det_test = _make_optional_dataset(
                        DetectDataset,
                        det_root,
                        source.get("test_images", "test/images"),
                    ...
                    ds = _apply_test_dataset_policy(
                        ds,
                        det_test,
                        key,
                        split_policy,
                        salt=source.get("split_salt", f"detect:{det_root}"),
                    )
                det_parts.append(ds)
                print(f"Eval detect[{key}]: {det_root / img_dir} / {det_root / lbl_dir} | {len(ds)}")
        if det_parts:
            ds = det_parts[0] if len(det_parts) == 1 else ConcatDataset(det_parts)
            ds = _maybe_trim(ds)
...
def evaluate_checkpoint(cfg, weights, device="cuda", split="val",
    ...
    data_cfg = cfg.get("data", {}) or {}
    attr_names = list((cfg.get("pose_attr", {}) or cfg.get("attributes", {}) or {}).get(
        "names",
    ...
    domain_cfg = cfg.get("domain_det", {}) or {}
    domain_pred_classes = [int(x) for x in domain_cfg.get(
        "eval_class_ids",
                ...
                for bi in range(len(preds)):
                    ...
                    if task == "attr":
                        ...
                        gt_persons["masks"].append(
                            ...
                            .astype(np.float32))
                        if 8 in domain_pred_classes:
                            attr_vals = batch_data["attrs"][bi].numpy()
                            attr_masks = batch_data["attr_mask"][bi].numpy()
                            if "falling" in attr_names:
                                falling_idx = attr_names.index("falling")
                                if falling_idx < attr_vals.shape[1] and falling_idx < attr_masks.shape[1]:
                                    falling_pos = person_mask & (attr_masks[:, falling_idx] > 0.0) & (attr_vals[:, falling_idx] >= 0.5)
                                    gts_by_image_class[(image_idx, 8)] = (
                                        gt_boxes[falling_pos].astype(np.float32)
                                    )
    ...
    domain_names = list(domain_cfg.get(
        "names", ["puddle", "fire", "smoke", "other", "helmet", "head", "cigarette", "falling"]))
    per_class = {}
    ...
    result = {
        "weights": str(weights),
        "config": str(Path(__file__).resolve().parent / "yaml/final_pose_attr_expanded_detect_vlm_mix_source_mask_smoking_aug_distill.yaml"),
        "split": split,
