...
from test_model.final_pose_attr_expanded_detect_falling.eval import (  # noqa: E402
    box_iou,
    build_eval_loaders,
    build_model,
    normalize_device,
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=str(
            PROJECT_ROOT
            / "test_model/final_pose_attr_expanded_detect_falling/yaml/"
            "final_pose_attr_expanded_detect_vlm_mix_source_mask_smoking_aug_distill.yaml"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Rewrite /root/autodl-tmp dataset roots to this local data root. "
            "For this repo use D:\\AI4PumpRoom\\data."
        ),
    )
    parser.add_argument(
        "--domain-thresholds",
    ...
    parser.add_argument(
        "--output",
        default="outputs/final_pose_attr_expanded_detect_falling_threshold_search",
    )
    return parser.parse_args()


def apply_local_data_root(cfg, local_data_root):
    if not local_data_root:
        return cfg
    root = Path(local_data_root).resolve()
    data_cfg = cfg.setdefault("data", {})

    def rewrite(value):
        text = str(value)
        mapping = {
            "/root/autodl-tmp/Detect": root / "Detect",
            "/root/autodl-tmp/Detect_head_helmet_cigarette": (
                root / "Detect_head_helmet_cigarette"
            ),
            "/root/autodl-tmp/Attr": root / "Attr",
            "/root/autodl-tmp/coco2017": root / "coco2017",
        }
        return str(mapping.get(text, value))

    for section_name in ("train", "val"):
        section = data_cfg.get(section_name, {}) or {}
        detect = section.get("detect", {}) or {}
        sources = detect.get("sources", [])
        if isinstance(sources, list):
            for source in sources:
                if isinstance(source, dict) and "root" in source:
                    source["root"] = rewrite(source["root"])
        attr = section.get("attr", {}) or {}
        if "root" in attr:
            attr["root"] = rewrite(attr["root"])
        pose = section.get("pose", {}) or {}
        if "root" in pose:
            pose["root"] = rewrite(pose["root"])
    return cfg
...
def summarize_rows(result, domain_names, attr_names):
    ...
    if row is None:
        lines.append("person\tNA\tNA\tNA\tNA\t0\t0")
    else:
        lines.append(
...
def main():
    ...
    cfg = apply_local_data_root(cfg, args.local_data_root)
