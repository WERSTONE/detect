...
def _class_mask(class_ids, num_classes=8):
...
def _load_manifest(root: Path, value):
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            "Run scripts/build_expanded_detect_falling_supervision_manifest.py first."
        )
    records = {}
...
class DetectDataset(BaseDetectDataset):
    def __init__(self, *args, domain_valid_class_ids=None,
                 supervision_manifest=None, domain_num_classes=8,
...
class AttrDataset(BaseAttrDataset):
    def __init__(self, *args, domain_valid_class_ids=None,
                 supervision_manifest=None, domain_num_classes=8, **kwargs):
        super().__init__(*args, **kwargs)
        self.domain_num_classes = int(domain_num_classes)
        self._static_domain_mask = _class_mask(domain_valid_class_ids or [1, 2, 3, 4, 7, 8], self.domain_num_classes)
        self._supervision_manifest = _load_manifest(self.root, supervision_manifest)
...
def build_final_train_loader(cfg: dict):
    ...
    attr_ds = AttrDataset(
        ...
        **_attr_dataset_options(train_cfg["attr"], split_policy),
        domain_valid_class_ids=train_cfg["attr"].get("domain_valid_class_ids", [1, 2, 3, 4, 7, 8]),
...
def build_final_val_loader(cfg: dict) -> FinalSequentialValLoader:
    ...
    if attr_root.exists():
        tasks["attr"] = AttrDataset(
            ...
            input_size=input_size,
            augment=False,
            domain_valid_class_ids=attr_cfg.get("domain_valid_class_ids", [1, 2, 3, 4, 7, 8]),
