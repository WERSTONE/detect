"""PyTorch webcam/image/video demo for the expanded-detect pose-attr model."""
...
from test_model.final_pose_attr_expanded_detect_falling.eval import build_model, normalize_device  # noqa: E402


IMAGE_EXTS = {".jpg", ".jpeg", ".png", ".bmp", ".webp"}
VIDEO_EXTS = {".mp4", ".avi", ".mov", ".mkv", ".wmv", ".m4v"}
CLASS_NAMES = {0: "person", 1: "puddle", 2: "fire", 3: "smoke", 4: "other", 5: "helmet", 6: "head", 7: "cigarette", 8: "falling"}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run expanded-detect final_pose_attr PyTorch inference on images, video, or webcam")
    ...
    parser.add_argument("--output", default="outputs/final_pose_attr_expanded_detect_pt", help="Output image/video/directory")
...
def draw_predictions(frame, pred, args, fps=None):
    ...
    attr_names = getattr(args, "attr_names", ATTR_NAMES)
...
def predictions_to_jsonable(pred, class_names=None, attr_names=None):
    class_names = class_names or CLASS_NAMES
    attr_names = attr_names or ATTR_NAMES
    items = []
    for box, score, cls_id, kpts, attrs in zip(
            pred["boxes"], pred["scores"], pred["classes"], pred["kpts"], pred["attrs"]):
        cls_id = int(cls_id)
...
def run_images(model, args, device):
    ...
    for image_path in image_paths:
        ...
        all_results[str(image_path)] = predictions_to_jsonable(
            pred,
            class_names=getattr(args, "class_names", CLASS_NAMES),
            attr_names=getattr(args, "attr_names", ATTR_NAMES),
        )
    ...
    try:
        while True:
            ...
            if jsonl_file is not None:
                payload = {
                    "frame": frame_idx,
                    "predictions": predictions_to_jsonable(
                        pred,
                        class_names=getattr(args, "class_names", CLASS_NAMES),
                        attr_names=getattr(args, "attr_names", ATTR_NAMES),
                    ),
            ...
            if writer is not None:
                writer.write(vis)
            if show_window:
                cv2.imshow("final_pose_attr_expanded_detect PyTorch", vis)
...
def main():
    ...
    domain_names = list((cfg.get("domain_det", {}) or {}).get("names", []))
    attr_names = list((cfg.get("pose_attr", {}) or {}).get("names", []))
    args.class_names = ["person"] + (domain_names or ["puddle", "fire", "smoke", "other"])
    args.attr_names = attr_names or ATTR_NAMES
    ...
    print(f"Classes: {', '.join(args.class_names)}")
    print(f"Attributes: {', '.join(args.attr_names)}")
