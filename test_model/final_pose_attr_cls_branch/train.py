"""Trainer entry for the cls-branch pose-anchor attribute experiment."""
...
from test_model.final_pose_attr_cls_branch.data import build_final_train_loader, build_final_val_loader
from test_model.final_pose_attr_cls_branch.model import create_final_pose_attr_model
from test_model.train.trainer import Trainer


def parse_args():
    parser = argparse.ArgumentParser(description="Final cls-branch pose-anchor attribute trainer")
    parser.add_argument(
        "--config",
        default=str(PROJECT_ROOT / "test_model" / "final_pose_attr_cls_branch" / "yaml" / "final_pose_attr_cls_branch_vlm_mix.yaml"),
    )
...
def main():
    ...
    base_save = Path(cfg["training"].get("save_dir", "checkpoints/final_pose_attr_cls_branch"))
    last_best = None
    ...
    if val_loader is not None and cfg.get("eval", {}).get("enabled", False):
        from test_model.final_pose_attr_cls_branch.eval import evaluate_checkpoint
