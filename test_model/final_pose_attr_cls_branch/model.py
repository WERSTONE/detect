...
class YOLOLikePoseAttrHead(nn.Module):
    ...
    def forward(self, features):
        ...
        for feat, cls_attr, kpt, action_branch in zip(
            ...
            attrs.append(
                torch.cat(
                    [cls_attr[:, 0:1], action[:, 0:1], action[:, 1:2], cls_attr[:, 1:2]],
                    dim=1,
                )
            )
        outs["kpt"] = kpts
...
def create_final_pose_attr_model(name="final_pose_attr_cls_branch", **kwargs):
    if name not in ("final_pose_attr", "bifpn_final_pose_attr", "final_pose_attr_cls_branch"):
        raise ValueError('Supported models: "final_pose_attr", "bifpn_final_pose_attr", "final_pose_attr_cls_branch".')
