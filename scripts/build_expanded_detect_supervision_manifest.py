        ...
        for dataset_dir in sorted(p for p in group_dir.iterdir() if p.is_dir()):
            for split in ("train", "val", "test"):
                ...
                for label_path in sorted(label_dir.glob("*.txt")):
                    ...
                    policy = "attr_smoking_positive" if smoking_positive else "attr_default"
                    records.append({
                        ...
                        "valid_class_ids": (
                            ATTR_SMOKING_POS_VALID if smoking_positive else ORIGINAL_VALID
                        ),
                    })
                    by_split[split] += 1
                    by_policy[policy] += 1
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--manifest-name", default="domain_supervision_manifest.jsonl")
